...
//Google - GMap in Bookmarks.html file
...
// WikiSearch
javascript: (function () { const input = document.createElement('input'); input.setAttribute('value', window.getSelection().toString()); document.body.appendChild(input); input.select(); document.execCommand('copy'); document.body.removeChild(input); navigator.clipboard.readText() .then((text) => { let topic = text.replace(/ /g, "_"); let wAddress = "https://it.wikipedia.org/wiki/" + topic; window.open(wAddress, "_blank"); }); })();


// AISearch
javascript: (function () { const input = document.createElement('input'); input.setAttribute('value', window.getSelection().toString()); document.body.appendChild(input); input.select(); document.execCommand('copy'); document.body.removeChild(input); navigator.clipboard.readText() .then((text) => { let topic = text.replace(/ /g, "+"); let intro = "Scrivi una breve spiegazione riguardo: "; let phAddress = "https://www.phind.com/search?q=" + intro + topic; window.open(phAddress, "_blank"); let yAddress = "https://you.com/search?q=" + intro + topic + "&fromSearchBar=true&tbm=youchat&chatMode=default"; window.open(yAddress, "_blank"); }); })();
